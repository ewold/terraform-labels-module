locals {
  ...
  services                       = "${concat(local.context_local["service"], list(""))}"
  service_context_or_default     = "${length(local.services[0]) > 0 ? local.services[0] : var.service}"
  service_or_context             = "${var.service != ""? var.service : local.service_context_or_default}"
  service                        = "${lower(replace(local.service_or_context, "/[^a-zA-Z0-9]/", ""))}"
  regions                        = "${concat(local.context_local["region"], list(""))}"
  region_context_or_default      = "${length(local.regions[0]) > 0 ? local.regions[0] : var.region}"
  region_or_context              = "${var.region != "" ? var.region : local.region_context_or_default}"
  region                         = "${lower(replace(local.region_or_context, "/[^a-zA-Z0-9]/", ""))}"
  ...
  generated_tags = "${zipmap(
    compact(list("Service", local.region != "" ? "Region" : "", local.environment != "" ? "Environment" : "", local.stage != "" ? "Stage" : "")),
    compact(list(local.id, local.region, local.environment, local.stage))
    )}"
  ...
  label_order_default_list = "${list("stage", "region", "environment", "service", "attributes")}"
